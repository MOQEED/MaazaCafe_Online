  ...
  const renderForm = () => (
    <form className="seller-card product-form" onSubmit={onSubmit}>
      <h2 className="seller-card-heading">
        {isEdit ? 'Edit Dish' : 'Add a Dish to Menu'}
      </h2>

      <div className="form-grid">
        {renderTextField('title', 'DISH NAME', {
          placeholder: 'e.g. Hyderabadi Chicken Dum Biryani',
        })}
        {renderTextField('brand', 'KITCHEN / BRAND', {
          placeholder: 'e.g. Maaza Kitchen',
        })}

        {renderSelectField('category', 'CATEGORY', CATEGORY_OPTIONS)}
        {renderTextField('price', 'PRICE (RS)', {placeholder: '180.00'})}

        {renderTextField('image_url', 'FOOD IMAGE URL', {
          placeholder: 'https://images.unsplash.com/...',
        })}
        {renderTextField('rating', 'RATING (0 - 5)', {placeholder: '4.8'})}
        ...
        {renderTextField('total_reviews', 'TOTAL REVIEWS', {placeholder: '25'})}
      </div>
      ...
      <div className="form-field">
        ...
        <textarea
          ...
          rows="4"
          placeholder="Describe flavor, key spices, sides (e.g. served with Mirchi Ka Salan)..."
      ...
      <label className="form-checkbox-row" htmlFor="is_prime">
        ...
        <span className="form-checkbox-text">
          Chef&apos;s Special / Prime Deal – highlighted on the specials banner and featured for logged-in food lovers.
        </span>
      </label>
      ...
      <div className="form-actions">
        <button
          ...
          disabled={isSaving}
        >
          {isSaving ? 'Saving…' : isEdit ? 'Update Dish' : 'Add Dish to Menu'}
        </button>
        <button
          type="button"
